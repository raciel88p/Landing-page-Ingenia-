...
export function WhatsAppButton() {
  ...
  return (
    <a
      ...
      className={`fixed bottom-6 right-6 z-50 flex items-center justify-center gap-3 px-4 py-3 rounded-full shadow-2xl transition-all duration-500 hover:scale-105 active:scale-95 ${
        visible
    ...
    >
      <span className="absolute inline-flex h-full w-full rounded-full opacity-40 animate-ping left-0 top-0" style={{ backgroundColor: "#25D366" }} />
      <svg
      ...
      <span className="text-white font-bold text-sm relative z-10 hidden sm:block">💬 Cotizar por WhatsApp</span>
    </a>
  );
}
